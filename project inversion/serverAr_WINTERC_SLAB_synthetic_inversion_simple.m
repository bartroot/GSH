...
region.ax = 5+resolution/2;
region.bx = -10-resolution/2;
region.cx = 95-resolution/2;
region.dx = 110+resolution/2;
region.bound = 0;
...
[mohoC,Lon,Lat] =            gmt2matrix(load('/Users/bartroot/PhD/Data/Leicester/High_resolution_CCrust/CCrust05.t15.gmt'));
...
elseif strcmp(ExperimentalCaseNumber,'E002')
    ...
    if bound_correct == 1           
        ...
        [bcx,dcx,dum] = find(Lat==region.bx-region.bound&Lon==region.dx+region.bound);
        
        redRC = zeros(size(rho_crust));
        redRL = zeros(size(rho_litho));
        redRM = zeros(size(rho_mantl));
        
        redRC(acx:bcx,ccx:dcx) = RC(acx:bcx,ccx:dcx).*varCm;
        redRL(acx:bcx,ccx:dcx) = RL(acx:bcx,ccx:dcx).*varLm;
        redRM(acx:bcx,ccx:dcx) = RM(acx:bcx,ccx:dcx).*varMm;
                
        irho_crust = rho_crust - redRC;
        irho_litho = rho_litho - redRL;
        irho_mantl = rho_mantl - redRM;
    else
        irho_crust = rho_crust - RC*varCm;
        irho_litho = rho_litho - RL*varLm;
        irho_mantl = rho_mantl - RM*varMm;
    end
...
for aaa = 1%:llist
	...
	%Adum = StatisticsV3.Pinv;
	fE = sqrt(diag(StatisticsV3.Pinv));
        %COR = StatisticsV3.Pinv/(fE*fE');
	%COR = int64(COR.*1000);
	save([directory 'Stats_' num2str(ite) '_' ExperimentalCaseNumber '.mat'],'fE')
	clear fE
